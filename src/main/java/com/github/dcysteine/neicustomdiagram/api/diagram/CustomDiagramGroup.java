...
/**
 * Extension of {@link DiagramGroup} that provides a way to specify custom behavior for on special IDs passed to NEI.
 *
 * <p>
 * This is useful for doing things like having a custom button that shows a different set of diagrams. You will need to
 * add in a custom button or some other way of invoking NEI with the custom ID.
 *
 * <p>
 * To avoid collisions with any other NEI handlers, it is strongly recommended that the diagram group's group ID be a
 * prefix of all custom behavior IDs.
 *
...
public class CustomDiagramGroup extends DiagramGroup {
     ...
     *
     * <p>
     * The custom diagram method will be called when NEI is invoked with the custom behavior ID.
     *
     * <p>
     * To avoid collisions with any other NEI handlers, it is strongly recommended that the diagram group's group ID be
