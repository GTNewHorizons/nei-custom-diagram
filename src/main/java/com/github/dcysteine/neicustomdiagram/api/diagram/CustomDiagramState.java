...
/**
 * Extension of {@link DiagramState} that provides an outline for handling state, both per-diagram and global (to the
 * diagram group).
 *
 * <p>
 * Rather than using this class directly, you probably want to write a subclass that provides a no-parameter constructor
 * (for passing to {@link DiagramGroup}'s constructor) and provides values for {@code T} and {@code U}.
 */
