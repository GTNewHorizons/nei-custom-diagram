 ...
 *
 * <p>
 * NEI gives us {@code 166px} width to work with. Slots take up {@code 18px} each, and we want to have a margin between
 * slots to give us room to draw lines. So we can construct a 7-column grid of slots with a {@code 6px} margin between
 * slots, for a total of {@code 7 * 18 + 6 * 6 = 160}.
 *
 * <p>
 * For height, it looks like there's just about exactly enough height to fit 14 rows of slots when NEI is maximized. At
 * the default Minecraft window height, NEI can only fit 6 rows of slots.
 *
 * <p>
 * For better resolution, we'll halve the width of each grid square. So the even-numbered grid indices correspond to
 * slot centers in the {@code 14 x 7} slot grid described above, and the odd-numbered grid indices correspond to the
 * midpoint between those slots. These midpoints can be used as margin points, or to construct offset slots.
 */
@AutoValue
public abstract class Grid {
    ...
    /**
     * Returns a {@link Point} corresponding to the margin of the grid square {@code (x, y)} in the specified direction.
     *
     * <p>
     * Grid coordinates are 0-indexed.
     */
    ...
    /**
     * Returns a {@link Point} corresponding to the edge of the grid square {@code (x, y)} in the specified direction.
     *
     * <p>
     * Grid coordinates are 0-indexed.
     */
    ...
    /**
     * Returns a {@link Point} corresponding to the edge of a big slot placed on the grid square {@code (x, y)} in the
     * specified direction.
     *
     * <p>
     * Grid coordinates are 0-indexed.
    ...
    /**
     * Returns a {@link Point} corresponding to the margin of the slot at position {@code pos} in the specified
     * direction.
     */
    ...
    /**
     * Returns a {@link Point} corresponding to the edge of the slot at position {@code pos} in the specified direction.
     */
    ...
    /**
     * Returns a {@link Point} corresponding to the edge of the big slot at position {@code pos} in the specified
     * direction.
     */
