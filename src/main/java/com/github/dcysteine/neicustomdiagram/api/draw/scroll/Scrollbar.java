...
class Scrollbar {
    ...
    /**
     * Returns the scroll offset, in pixels. Will be a non-negative integer between 0 and the maximum scroll amount,
     * inclusive.
     */
    ...
    public void scrollToMouse() {
        int mousePos = orientation.dotProduct(scrollManager.getAbsoluteMousePosition());
        int mouseOffset = mousePos - orientation.dotProduct(dimensions.anchor) - dimensions.cursorLength / 2;
     ...
     *
     * <p>
     * {@link #scroll(ScrollDirection, int)} does not check bounds, because we will check them here.
     */
    ...
    public void draw() {
        ...
        int scrollbarAboveLength = scroll * dimensions.scrollableLength / dimensions.offscreenLength;
        // The top-left corner of the scrollbar above the cursor.
        Point scrollbarAboveStart = dimensions.anchor.translate(orientation.transposeScale(-SCROLLBAR_RADIUS));
        Dimension scrollbarAboveDimension = Dimension.create(scrollbarAboveLength, 2 * SCROLLBAR_RADIUS);
        ...
        int scrollbarBelowLength = dimensions.scrollbarLength - (scrollbarAboveLength + dimensions.cursorLength);
        // The top-left corner of the scrollbar below the cursor.
        Point scrollbarBelowStart = dimensions.end.translate(orientation.scale(-scrollbarBelowLength));
        scrollbarBelowStart = scrollbarBelowStart.translate(orientation.transposeScale(-SCROLLBAR_RADIUS));
        Dimension scrollbarBelowDimension = Dimension.create(scrollbarBelowLength, 2 * SCROLLBAR_RADIUS);
        ...
        if (scrollbarAboveLength > 0) {
            GuiDraw.drawRect(
                    scrollbarAboveStart.x(),
                    scrollbarAboveStart.y(),
                    scrollbarAboveWidth,
                    scrollbarAboveHeight,
                    bgColour);
        }

        GuiDraw.drawRect(cursorStart.x(), cursorStart.y(), cursorWidth, cursorHeight, fgColour);

        if (scrollbarBelowLength > 0) {
            GuiDraw.drawRect(
                    scrollbarBelowStart.x(),
                    scrollbarBelowStart.y(),
                    scrollbarBelowWidth,
                    scrollbarBelowHeight,
                    bgColour);
        }
    }
     ...
     *
     * <p>
     * We'll re-compute these values each time the diagram is rendered, and then just store them, to avoid needlessly
     * re-computing them.
     */
    private class ScrollbarDimensions {
        ...
        public void compute(Dimension diagramDimension) {
            ...
            anchor = viewportPos
                    .translate(orientation.transposeScale(orientation.transposeProduct(viewportDim) + OFFSET));
            ...
            cursorLength = Math.max((viewportLength * scrollbarLength) / diagramLength, MIN_CURSOR_LENGTH);
