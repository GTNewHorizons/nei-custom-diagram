...
public final class NbtViewer implements DiagramGenerator {
    ...
    public NbtViewer(String groupId) {
        this.info = DiagramGroupInfo.builder(Lang.NBT_VIEWER.trans("groupname"), groupId, ICON, 1).setIgnoreNbt(false)
                .setDefaultVisibility(DiagramGroupVisibility.DISABLED)
                .setDescription("This diagram pretty-prints NBT, for convenient viewing.").build();
    }
    ...
    private Collection<Diagram> generateDiagrams(Interactable.RecipeType recipeType, Component component) {
        ...
        return Lists.newArrayList(
                Diagram.builder().addLayout(buildLayout(component.nbt().get()))
                        .insertIntoSlot(Layout.SlotKey.create(SLOT_KEY), DisplayComponent.builder(component).build())
                        .build());
    }
