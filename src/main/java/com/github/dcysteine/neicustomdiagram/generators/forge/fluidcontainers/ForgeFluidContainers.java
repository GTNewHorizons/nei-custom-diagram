 ...
 *
 * <p>
 * This diagram group also supports showing all fluids that an empty fluid container can contain.
 */
public final class ForgeFluidContainers implements DiagramGenerator {
    ...
    public ForgeFluidContainers(String groupId) {
        this.info = DiagramGroupInfo.builder(Lang.FORGE_FLUID_CONTAINERS.trans("groupname"), groupId, ICON, 2)
                ...
                .setDescription("This diagram displays Forge registered fluids" + " and fluid containers.").build();
    }
    ...
    public DiagramGroup generate() {
        ...
        SetMultimap<ItemComponent, Diagram> emptyContainersSetMultimap = MultimapBuilder.hashKeys().hashSetValues()
                .build();
    ...
    private Diagram generateDiagram(FluidComponent fluid, SetMultimap<ItemComponent, Diagram> emptyContainersMultimap) {
        ...
        fluidsBuilder.insertIntoNextSlot(
                DisplayComponent.builder(fluid).setAdditionalTooltip(
                        Tooltip.builder().setFormatting(Tooltip.SLOT_FORMATTING)
                                .addTextLine(Lang.FORGE_FLUID_CONTAINERS.trans("fluidlabel")).addSpacing()
                                .setFormatting(Tooltip.INFO_FORMATTING)
                                .addTextLine(
                                        Lang.FORGE_FLUID_CONTAINERS.transf("fluidnamelabel", fluid.fluid().getName()))
                                .build())
                        .build());

        Optional<ItemComponent> blockOptional = FluidDictUtil.fluidToItem(fluid);
        blockOptional.ifPresent(
                block -> fluidsBuilder.insertIntoNextSlot(
                        DisplayComponent.builder(block)
                                .setAdditionalTooltip(
                                        Tooltip.create(
                                                Lang.FORGE_FLUID_CONTAINERS.trans("blocklabel"),
            ...
            displayItemOptional.ifPresent(
                    displayItem -> fluidsBuilder.insertIntoNextSlot(
                            DisplayComponent.builder(displayItem)
                                    .setAdditionalTooltip(
                                            Tooltip.create(
                                                    Lang.FORGE_FLUID_CONTAINERS.trans("gregtechitemlabel"),
                                                    Tooltip.SLOT_FORMATTING))
                                    .build()));
        }
    ...
    private Layout buildLayout() {
        return Layout.builder().addInteractable(new AllDiagramsButton(info, Grid.GRID.grid(0, 0)))
                .putSlotGroup(
                        SLOT_GROUP_FLUIDS,
                        SlotGroup.builder(1, 6, Grid.GRID.grid(1, 2), Grid.Direction.S).build())
                .putSlotGroup(
