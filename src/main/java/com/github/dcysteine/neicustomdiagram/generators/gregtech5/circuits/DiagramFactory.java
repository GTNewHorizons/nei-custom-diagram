...
class DiagramFactory {
    ...
    DiagramFactory(
            CircuitLineHandler circuitLineHandler,
    ...
    Diagram buildOverviewDiagram() {
        ...
        for (int i = 0; i < circuitLines.size(); i++) {
            ...
            for (int j = 0; j < circuitLine.circuits().size(); j++) {
                circuitLinesSlotSubBuilder.insertIntoSlot(
                        i, circuitLine.startTier() + j + 1, buildCircuitDisplayComponent(circuitLine, j));
            }
        }

        Diagram.Builder.SlotGroupManualSubBuilder individualCircuitsSlotSubBuilder =
                diagramBuilder.manualInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.INDIVIDUAL_CIRCUITS);
        circuitLines = circuitLineHandler.individualCircuits();
        for (int i = 0; i < circuitLines.size(); i++) {
            CircuitLine circuitLine = circuitLines.get(i);
            Preconditions.checkState(
                    circuitLine.circuits().size() == 1, "Expected individual circuit: %s", circuitLine);
    ...
    void buildDiagram(
            ...
            int recipeIndex,
            int recipeCount,
            Optional<CircuitRecipe> recipeOptional,
        ...
        if (recipeHandler.hasCraftingTableRecipes(circuit)) {
            diagramBuilder.addInteractable(labelHandler.buildLabel(
        ...
        Diagram.Builder.SlotGroupManualSubBuilder circuitLineCircuitsSlotSubBuilder =
                diagramBuilder.manualInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.CIRCUIT_LINE_CIRCUITS);
        circuitLineCircuits
                .previousCircuit()
        ...
        diagramBuilder
                .autoInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.TIER_CIRCUITS)
        ...
        if (recipeOptional.isPresent()) {
            ...
            if (recipe.missingCombinations()) {
                ...
                drawForeground = position ->
                        Draw.drawTextOverIcon("*", position, Grid.Direction.NW, Draw.Colour.RED, true, true);
            }
            ...
            if (recipe.requiresCleanroom()) {
                diagramBuilder.addInteractable(labelHandler.buildLabel(
                        LabelHandler.ItemLabel.CLEAN_ROOM, LayoutHandler.AdditionalRecipeLabelPositions.CLEAN_ROOM));
            }
            if (recipe.requiresLowGravity()) {
                diagramBuilder.addInteractable(labelHandler.buildLabel(
                        LabelHandler.ItemLabel.LOW_GRAVITY, LayoutHandler.AdditionalRecipeLabelPositions.LOW_GRAVITY));
            }
            ...
            for (int i = 0; i < itemInputs.size(); i++) {
                ...
                diagramBuilder
                        .autoInsertIntoSlotGroup(LayoutHandler.SLOT_GROUP_RECIPE_ITEM_INPUTS.get(i))
            ...
            diagramBuilder
                    .autoInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.RECIPE_FLUID_INPUTS)
