...
class DiagramFactory {
    ...
    DiagramFactory(CircuitLineHandler circuitLineHandler, LabelHandler labelHandler, LayoutHandler layoutHandler,
    ...
    Diagram buildOverviewDiagram() {
        Diagram.Builder diagramBuilder = Diagram.builder().addLayout(layoutHandler.overviewLayout());

        Diagram.Builder.SlotGroupManualSubBuilder circuitLinesSlotSubBuilder = diagramBuilder
                .manualInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.CIRCUIT_LINES);
        ...
        for (int i = 0; i < circuitLines.size(); i++) {
            ...
            if (!circuitLine.boards().isEmpty()) {
                circuitLinesSlotSubBuilder
                        .insertIntoSlot(i, 0, ComponentTransformer.transformToDisplay(circuitLine.boards()));
            }

            for (int j = 0; j < circuitLine.circuits().size(); j++) {
                circuitLinesSlotSubBuilder.insertIntoSlot(
                        i,
                        circuitLine.startTier() + j + 1,
                        buildCircuitDisplayComponent(circuitLine, j));
            }
        }

        Diagram.Builder.SlotGroupManualSubBuilder individualCircuitsSlotSubBuilder = diagramBuilder
                .manualInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.INDIVIDUAL_CIRCUITS);
        circuitLines = circuitLineHandler.individualCircuits();
        for (int i = 0; i < circuitLines.size(); i++) {
            CircuitLine circuitLine = circuitLines.get(i);
            Preconditions
                    .checkState(circuitLine.circuits().size() == 1, "Expected individual circuit: %s", circuitLine);

            if (!circuitLine.boards().isEmpty()) {
                individualCircuitsSlotSubBuilder
                        .insertIntoSlot(i, 0, ComponentTransformer.transformToDisplay(circuitLine.boards()));
            }

            individualCircuitsSlotSubBuilder.insertIntoSlot(i, 1, buildCircuitDisplayComponent(circuitLine, 0));
        }

        Diagram.Builder.SlotGroupManualSubBuilder circuitPartsSlotSubBuilder = diagramBuilder
                .manualInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.CIRCUIT_PARTS);
        ImmutableList<ImmutableList<ItemComponent>> circuitParts = circuitLineHandler.circuitParts();
        for (int i = 0; i < circuitParts.size(); i++) {
            ...
            for (int j = 0; j < circuitPartsSubList.size(); j++) {
                circuitPartsSlotSubBuilder
                        .insertIntoSlot(i, j, ComponentTransformer.transformToDisplay(circuitPartsSubList.get(j)));
            }
        }
    ...
    void buildDiagrams(ItemComponent circuit, ComponentDiagramMatcher.Builder matcherBuilder) {
        ImmutableList<CircuitRecipe> recipes = recipeHandler.getCircuitAssemblingMachineRecipes(circuit);
    ...
    void buildDiagram(ItemComponent circuit, int recipeIndex, int recipeCount, Optional<CircuitRecipe> recipeOptional,
        ...
        CircuitLineHandler.CircuitLineCircuits circuitLineCircuits = circuitLineHandler.circuitLineCircuits(circuit);
        Diagram.Builder.SlotGroupManualSubBuilder circuitLineCircuitsSlotSubBuilder = diagramBuilder
                .manualInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.CIRCUIT_LINE_CIRCUITS);
        circuitLineCircuits.previousCircuit()
                .ifPresent(previousCircuit -> circuitLineCircuitsSlotSubBuilder.insertIntoSlot(0, 0, previousCircuit));
        circuitLineCircuits.currentCircuit()
                .ifPresent(currentCircuit -> circuitLineCircuitsSlotSubBuilder.insertIntoSlot(1, 0, currentCircuit));
        circuitLineCircuits.nextCircuit()
                .ifPresent(nextCircuit -> circuitLineCircuitsSlotSubBuilder.insertIntoSlot(2, 0, nextCircuit));
        ...
        if (recipeOptional.isPresent()) {
            ...
            CustomInteractable baseLabel = labelHandler.buildLabel(
                    LabelHandler.ItemLabel.CIRCUIT_ASSEMBLING_MACHINE,
                    LayoutHandler.AdditionalRecipeLabelPositions.CIRCUIT_ASSEMBLING_MACHINE);
            Tooltip.Builder tooltipBuilder = Tooltip.builder().addAllLines(baseLabel.tooltip().lines()).addSpacing()
                    .addTextLine(Lang.GREGTECH_5_CIRCUITS.transf("recipeindexlabel", recipeIndex + 1, recipeCount));
            ...
            if (recipe.missingCombinations()) {
                ...
                drawForeground = position -> Draw
                        .drawTextOverIcon("*", position, Grid.Direction.NW, Draw.Colour.RED, true, true);
            }
            diagramBuilder.addInteractable(
                    CustomInteractable.builder(baseLabel.drawable()).setTooltip(tooltipBuilder.build())
                            .setDrawForeground(drawForeground).build());
            ...
            for (int i = 0; i < itemInputs.size(); i++) {
                ...
                diagramBuilder.autoInsertIntoSlotGroup(LayoutHandler.SLOT_GROUP_RECIPE_ITEM_INPUTS.get(i))
                        .insertEachSafe(subItemInputs);
                subItemInputs.stream().map(DisplayComponent::component)
    ...
    static DisplayComponent buildCircuitDisplayComponent(CircuitLine circuitLine, int index) {
        return GregTechCircuits
                .buildCircuitDisplayComponent(circuitLine.circuits().get(index), circuitLine.startTier() + index);
    }
}
