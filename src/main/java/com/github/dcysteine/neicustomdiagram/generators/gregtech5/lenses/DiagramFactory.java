...
class DiagramFactory {
    ...
    void buildDiagrams(RecipeHandler.Lens lens, ComponentDiagramMatcher.Builder matcherBuilder) {
        List<RecipeHandler.Recipe> recipes = Lists.newArrayList(recipeHandler.recipes(lens));

        // Sort so that lens-specific recipes are shown first.
        Comparator<RecipeHandler.Recipe> recipeComparator = Comparator.<RecipeHandler.Recipe, Boolean>comparing(
                recipe -> recipeHandler.isColourRecipe(lens.colour(), recipe)).thenComparing(Comparator.naturalOrder());
        recipes.sort(recipeComparator);

        List<List<RecipeHandler.Recipe>> partitionedRecipes = Lists
                .partition(recipes, LayoutHandler.MAX_RECIPES_PER_PAGE);
        partitionedRecipes.forEach(partition -> buildDiagram(lens, matcherBuilder, partition));
    }

    void buildDiagram(RecipeHandler.Lens lens, ComponentDiagramMatcher.Builder matcherBuilder,
        ...
        Diagram.Builder diagramBuilder = Diagram.builder().addAllLayouts(layoutHandler.requiredLayouts())
                .addAllOptionalLayouts(layoutHandler.optionalLayouts()).addInteractable(lens.colour().buildLabel());
        ...
        Diagram.Builder.SlotGroupAutoSubBuilder slotGroupSubBuilder = diagramBuilder
                .autoInsertIntoSlotGroup(LayoutHandler.SlotGroupKeys.SAME_COLOUR_LENSES);
        for (RecipeHandler.Lens sameColourLens : recipeHandler.lenses(lens.colour())) {
            ...
            if (sameColourLens.equals(lens)) {
                displayComponentBuilder.setAdditionalInfo("*").setAdditionalTooltip(
                        Tooltip.create(Lang.GREGTECH_5_LENSES.trans("currentlenslabel"), Tooltip.INFO_FORMATTING));
            }
        ...
        for (int i = 0; i < recipes.size(); i++) {
            ...
            if (recipeHandler.isColourRecipe(lens.colour(), recipe)) {
                inputBuilder.setAdditionalTooltip(
                        Tooltip.create(
                                Lang.GREGTECH_5_LENSES.transf("colourrecipelabel", lens.colour().translateColour()),
                                Tooltip.INFO_FORMATTING));
            } else {
                inputBuilder.setAdditionalInfo("*").setAdditionalTooltip(
                        Tooltip.create(Lang.GREGTECH_5_LENSES.trans("specificrecipelabel"), Tooltip.INFO_FORMATTING));
            }

            diagramBuilder.insertIntoSlot(LayoutHandler.SLOT_RECIPE_INPUTS.get(i), inputBuilder.build());
            diagramBuilder.insertIntoSlot(LayoutHandler.SLOT_RECIPE_OUTPUTS.get(i), recipe.output());
