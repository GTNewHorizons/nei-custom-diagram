...
class LayoutHandler {
    ...
    static final ImmutableList<Layout.SlotKey> SLOT_RECIPE_INPUTS = ImmutableList.copyOf(
            IntStream.range(0, MAX_RECIPES_PER_PAGE).mapToObj(i -> Layout.SlotKey.create("recipe-input-" + i))
                    .collect(Collectors.toList()));
    static final ImmutableList<Layout.SlotKey> SLOT_RECIPE_OUTPUTS = ImmutableList.copyOf(
            IntStream.range(0, MAX_RECIPES_PER_PAGE).mapToObj(i -> Layout.SlotKey.create("recipe-output-" + i))
                    .collect(Collectors.toList()));
    ...
    void initialize() {
        ...
        optionalLayouts = ImmutableList.copyOf(
                IntStream.range(0, MAX_RECIPES_PER_PAGE).mapToObj(this::buildRecipeLayout)
                        .collect(Collectors.toList()));
    }
    ...
    private Layout buildHeaderLayout() {
        return Layout.builder().addInteractable(new AllDiagramsButton(info, Grid.GRID.grid(0, 0))).putSlotGroup(
                SlotGroupKeys.SAME_COLOUR_LENSES,
                SlotGroup.builder(9, 1, Grid.GRID.grid(6, 2), Grid.Direction.C).setDefaultTooltip(
                        Tooltip.create(Lang.GREGTECH_5_LENSES.trans("samecolourlensesslot"), Tooltip.SLOT_FORMATTING))
                        .build())
                .build();
    }
        ...
        return Layout.builder()
                .addLines(
                        Lines.builder(Grid.GRID.grid(inputX, gridY))
                                .addArrow(Grid.GRID.edge(outputX, gridY, Grid.Direction.W)).build())
                .putSlot(
