...
public final class GregTechOreDictionary implements DiagramGenerator {
    ...
    private static Layout buildLayout(Optional<ItemData> itemDataOptional) {
        Layout.Builder builder = Layout.builder().putSlotGroup(
                SLOT_GROUP_UNIFY,
                SlotGroup.builder(2, 6, Grid.GRID.grid(1, 3), Grid.Direction.S).setDefaultTooltip(
                        Tooltip.create(Lang.GREGTECH_5_ORE_DICTIONARY.trans("unifyslot"), Tooltip.SLOT_FORMATTING))
                        .build())
                .putSlotGroup(
                        SLOT_GROUP_ALL,
        ...
        if (itemDataOptional.isPresent() && itemDataOptional.get().mMaterial != null) {
            ...
            Text materialNameText = Text.builder(materialName, Grid.GRID.grid(6, 0), Grid.Direction.C)
                    .setSmall(materialSmall).build();
            Interactable materialNameLabel = CustomInteractable.builder(materialNameText).setTooltip(
                    Tooltip.create(Lang.GREGTECH_5_ORE_DICTIONARY.trans("materialnamelabel"), Tooltip.SLOT_FORMATTING))
                    .build();

            Text prefixNameText = Text.builder(prefixName, Grid.GRID.grid(6, 1), Grid.Direction.C).setSmall(prefixSmall)
                    .build();
            Interactable prefixNameLabel = CustomInteractable.builder(prefixNameText).setTooltip(
                    Tooltip.create(Lang.GREGTECH_5_ORE_DICTIONARY.trans("prefixnamelabel"), Tooltip.SLOT_FORMATTING))
                    .build();
