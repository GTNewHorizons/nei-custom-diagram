...
public final class GregTechRecipeDebugger implements DiagramGenerator {
    ...
    public enum View {
        ...
        UNNECESSARY_CIRCUIT_RECIPES("-unnecessary-circuit-recipes", ItemComponent.create(Items.sugar, 0),
                "unnecessarycircuitrecipesbutton"),

        COLLIDING_RECIPES("-colliding-recipes", ItemComponent.create(Items.slime_ball, 0), "collidingrecipesbutton"),

        VOIDING_RECIPES("-voiding-recipes", ItemComponent.create(Items.magma_cream, 0), "voidingrecipesbutton"),

        UNEQUAL_CELL_RECIPES("-unequal-cell-recipes", GregTechOreDictUtil.getComponent(ItemList.Cell_Empty),
    ...
    public CustomDiagramGroup generate() {
        ...
        ImmutableMap.Builder<String, Supplier<Collection<Diagram>>> customBehaviorMapBuilder = ImmutableMap.builder();
        Arrays.stream(View.values()).forEach(
                view -> customBehaviorMapBuilder.put(view.behaviorId(info), () -> diagramHandler.getDiagrams(view)));
