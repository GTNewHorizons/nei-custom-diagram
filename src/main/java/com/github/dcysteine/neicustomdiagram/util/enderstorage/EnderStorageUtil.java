...
public final class EnderStorageUtil {
    ...
    public static Map<EnderStorageFrequency, EnderItemStorage> getEnderChests(Owner owner) {
        ...
        IntStream.rangeClosed(0, MAX_FREQUENCY).forEach(
                freq -> map.put(
                        ...
                        (EnderItemStorage) getManager().getStorage(ownerParam, freq, Type.CHEST.stringParam)));
    ...
    public static Map<EnderStorageFrequency, EnderLiquidStorage> getEnderTanks(Owner owner) {
        ...
        IntStream.rangeClosed(0, MAX_FREQUENCY)
                ...
                .map(EnderStorageUtil::reverseInt).forEach(
                        freq -> map.put(
                                EnderStorageFrequency.createReverse(freq),
                                (EnderLiquidStorage) getManager().getStorage(ownerParam, freq, Type.TANK.stringParam)));

        return map;
    }

    /**
     * Ender tanks seem to have their frequency colours in reverse order; this method helps us compensate for that.
     */
    ...
    /**
     * Try to return the server instance when possible (which is for the host of a single-player or LAN world).
     *
     * <p>
     * If we're playing on a server, the server instance won't work, so we'll return the client instance, but it has
     * access to limited data. As of the time of this writing ({@code 2021-09}), the client instance will only refresh
     * data for ender chests when they are opened by the player, and data for ender tanks is missing entirely.
     *
     * @see <a href=https://github.com/GTNewHorizons/GT-New-Horizons-Modpack/issues/7860#issuecomment-925382503> GitHub
     *      issue</a>
     */
